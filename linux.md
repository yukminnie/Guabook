...
VPS购买, http://www.vultr.com/?ref=6991688
...
CentOS 内核老旧，软件安装麻烦，抄的redhat
```

**连接服务器**

```
连接服务器推荐使用 bitvise 软件(windows下)，可以连接，可以上传文件
如果在 mac 下, 直接用终端连接
```
...
```
    ...
    more less head tail
    more 可以分屏分批看文件内容
    less 比 more 更高级，可以前后退看文件（j，k上下推送）
    head 可以显示文件的前 10 行
    tail 可以显示文件的后 10 行
...
proc run srv tmp var lost+found mnt root sbin sys usr vmlinuz  暂时不用关心
```
...
kill 和 killall 杀进程
    ...
    killall 是用进程名字来杀进程，开多少杀多少，有危险性
